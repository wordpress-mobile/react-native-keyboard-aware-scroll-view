...
function KeyboardAwareHOC(
  ...
  return class
    ...
    async _calculateTopBottomDistance() {
   ...
   _updateKeyboardSpace = async (keyboardEvent: Object) => {
      await this._calculateTopBottomDistance();
            ...
            if (isAncestor) {
              UIManager.measureSelectionInWindow(
                ...
                (error: ?string, fieldX: number, fieldY: number, fieldWidth: number, fieldHeight: number, 
                  caretX: number, caretY: number, caretWidth: number, caretHeight: number) => {
                  if ( error ) {
                    //Try old way
    ...
    _extraScrollHeight() {
      return this.props.extraScrollHeight + this.topDistanceToWindow + this.bottomDistanceToWindow;
    }
